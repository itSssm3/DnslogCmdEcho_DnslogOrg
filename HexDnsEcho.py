...
# author: itSssm3 //https://github.com/itSssm3/DnslogCmdEcho_DnslogOrg
...
# NOTE ".count('.')" will change with the Dns-Platform-Url changed
# get DNSLog data 
def get_dnslogdata() -> list:
    if commandStartPos and commandEndFlag: 
        commandHex[commandName].extend([result[length-1][1]['subdomain'] 
                                        ...
                                        if result[length-1][1]['subdomain'].count('.') == 5])
...
if __name__ == '__main__':
    ...
    while True:
        ...
        # NOTE ".count('.')" will change with the Dns-Platform-Url changed
        if not commandStartPos and ((result[-1][1]['subdomain'].count('.'))  == 5 or 
                                    commandStartFlag): 
        ...
        if commandStartPos and ((result[-1][1]['subdomain'].count('.')) != 5 or 
                                commandEndFlag):
